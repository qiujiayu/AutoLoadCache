...
public abstract class AbstractCacheManager implements ICacheManager {
	 ...
	 * @return String 缓存Key
	 */
	private CacheKeyTO getCacheKey(InterceptorChain chain, Cache cache) {
		String className = chain.getCallingMethod().getDeclaringClass().getName();
	 ...
	 * @return 缓存Key
	 */
	private CacheKeyTO getCacheKey(InterceptorChain chain, Cache cache, Object result) {
		String className = chain.getCallingMethod().getDeclaringClass().getName();
		String methodName = chain.getCallingMethod().getName();
		Object[] arguments = chain.getArgs();
		String _key = cache.key();
	 ...
	 * @return 缓存Key
	 */
	private CacheKeyTO getCacheKey(InterceptorChain chain, AutoLoadTO autoLoadTO, ExCache cache, Object result) {
		String className = chain.getCallingMethod().getDeclaringClass().getName();
		String methodName = chain.getCallingMethod().getName();
		Object[] arguments = chain.getArgs();
		if (null != autoLoadTO) {
	 ...
	 * @return 缓存Key
	 */
	private CacheKeyTO getCacheKey(InterceptorChain chain, CacheDeleteKey cacheDeleteKey, Object retVal) {
		String className = chain.getCallingMethod().getDeclaringClass().getName();
		String methodName = chain.getCallingMethod().getName();
		Object[] arguments = chain.getArgs();
	 ...
	 *             异常
	 */
	public Object proceed(InterceptorChain chain, Cache cache) throws Throwable {
		Object[] arguments = chain.getArgs();
		if (null != cache.opType() && cache.opType() == CacheOpType.WRITE) {// 更新缓存操作
			Object result = getData(chain, null);
			if (CacheUtil.isCacheable(cache, arguments, result)) {
				CacheKeyTO cacheKey = getCacheKey(chain, cache, result);
				writeCache(chain, null, cache, cacheKey, result);
			}
			return result;
		}
		if (!CacheUtil.isCacheable(cache, arguments)) {// 如果不进行缓存，则直接返回数据
			return getData(chain, null);
		}
		CacheKeyTO cacheKey = getCacheKey(chain, cache);
		if (null == cacheKey) {
			return getData(chain, null);
		}
		...
		if (CacheUtil.isAutoload(cache, arguments)) {
			...
			if (null == autoLoadTO) {
				AutoLoadTO tmp = autoLoadHandler.putIfAbsent(cacheKey, chain, cache, serializer);
				if (null != tmp) {
					autoLoadTO = tmp;
				}
			}
		...
		return loadData(chain, autoLoadTO, cacheKey, cache);// 从DAO加载数据
	}
	 ...
	 * @return CacheWrapper
	 */
	private CacheWrapper writeCache(InterceptorChain chain, AutoLoadTO autoLoadTO, Cache cache, CacheKeyTO cacheKey, Object result) {
		if (null == cacheKey) {
			return null;
		}
		...
		if (null != exCaches && exCaches.length > 0) {
			...
			for (ExCache exCache : exCaches) {
				...
				CacheKeyTO exCacheKey = getCacheKey(chain, autoLoadTO, exCache, result);
				if (null == exCacheKey) {
					continue;
				}
	 ...
	 */
	@Override
	public Object loadData(InterceptorChain chain, AutoLoadTO autoLoadTO, CacheKeyTO cacheKey, Cache cache) throws Throwable {
		...
		if (null == isProcessing) {
			...
			if (null != _isProcessing) {
				isProcessing = _isProcessing;// 获取到第一个线程的ProcessingTO
												// 的引用，保证所有请求都指向同一个引用
			}
		}
		Object lock = null;
		...
		if (null == isProcessing) {
			...
			try {
				...
				result = getData(chain, autoLoadTO);
				CacheWrapper cacheWrapper = writeCache(chain, autoLoadTO, cache, cacheKey, result);
			...
			do {// 等待
				...
				if (isProcessing.isFirstFinished()) {
					cacheWrapper = isProcessing.getCache();// 从本地缓存获取数据，
															// 防止频繁去缓存服务器取数据，造成缓存服务器压力过大
					// System.out.println(tname + " do FirstFinished" +
					// " is null :" + (null == cacheWrapper));
			...
			try {
				result = getData(chain, autoLoadTO);
				writeCache(chain, autoLoadTO, cache, cacheKey, result);
			} catch (Throwable e) {
				throw e;
			} finally {
				synchronized (lock) {
	...
	private Object getData(InterceptorChain chain, AutoLoadTO autoLoadTO) throws Throwable {
		try {
			long startTime = System.currentTimeMillis();
			chain.doChain();
			Object result = chain.getReturn();
	 ...
	 *            返回值
	 */
	public void deleteCache(InterceptorChain chain, CacheDelete cacheDelete, Object retVal) {
		Object[] arguments = chain.getArgs();
		...
		for (int i = 0; i < keys.length; i++) {
			...
			CacheKeyTO key = getCacheKey(chain, keyConfig, retVal);
			if (null != key) {
				this.delete(key);
			}
		}
